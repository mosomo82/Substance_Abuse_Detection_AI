...
def load_analysis_inputs(
    ...
    ensemble = pd.read_csv(ENSEMBLE_CSV)[
        [c for c in ["post_id", "final_risk_level", "ensemble_confidence"]
         if c in pd.read_csv(ENSEMBLE_CSV, nrows=0).columns]
    ]
    ensemble = ensemble.rename(columns={"final_risk_level": "risk_level"})

    # ── join ──────────────────────────────────────────────────────────────────
    merged = posts.merge(ensemble, on="post_id", how="inner")
    ...
    emb_df     = pd.DataFrame({
        "post_id":   emb_table.column("post_id").to_pylist(),
        "embedding": emb_table.column("embedding").to_pylist(),
    })

    # Align embedding rows to the merged DataFrame order
    merged = merged.merge(
        emb_df.rename(columns={"embedding": "_emb"}),
        on="post_id",
        how="inner",
