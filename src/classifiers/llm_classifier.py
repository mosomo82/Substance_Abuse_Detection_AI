...
try:
    ...
    _client  = _genai_pkg.Client(api_key=_API_KEY) if _API_KEY else None
...
def classify_post_llm(post_text: str,
    ...
    if not _GEMINI_AVAILABLE:
        return {**_FALLBACK_RESULT,
                "reasoning": "GOOGLE_API_KEY not set or google-genai not installed."}

    # Build history as plain-text prefix for the new client API
    history_text = ""
...
def classify_corpus_hybrid(processed_df: pd.DataFrame,
    ...
    # Cap LLM calls to stay within free-tier quota (~200 req/day limit for comparison)
    _LLM_CAP = 200
    import random as _random
    if len(llm_queue) > _LLM_CAP:
        llm_queue = _random.sample(llm_queue, _LLM_CAP)
        print(f"  (sampled down to {_LLM_CAP} for LLM calls -- free-tier quota)")
...
def main() -> None:
    ...
    if not RULE_CSV.exists():
        ...
        if RAW.joinpath("seed_bank.pkl").exists():
            ...
            # Validate format: score_post_embedding needs risk-level keys
            # ('high', 'medium', 'low').  Substance-keyed banks are not usable.
            _risk_keys = {"high", "medium", "low"}
            if not _risk_keys.intersection(seed_bank.keys()):
                print(f"  Seed bank is substance-keyed {list(seed_bank.keys())} "
                      f"-- skipping embedding stage (needs risk-level keys).")
                seed_bank = None
            else:
